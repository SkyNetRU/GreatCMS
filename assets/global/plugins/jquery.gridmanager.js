          ...
          var canvas=gm.$el.find("#" + gm.options.canvasId);
           ...
           gm.$el.on("click", ".gm-preview", function(){
            ...
            /* Row settings */
               ...
               gm.prepareSettingsModal(row, 'row');
               gm.minicolor();
               $('#row_settings').modal('show');
               $('#row_settings').find('.save').off();
               $('#row_settings').find('.save').click(function () {
                   gm.saveSettings(row, 'row');
                   $('#row_settings').modal('hide');
               });
            ...
            // Change Row ID via rowsettings
               ...
               // Change Row ID via rowsettings
            }).on("blur", "input.gm-rowSettingsCLASS", function(){
                var row=$(this).closest(gm.options.rowSelector);
                row.attr("class", gm.options.rowClass + " " + gm.options.gmEditClass + " ui-sortable " + $(this).val());

           //Toggle Container Fluid
           }).on("change", "input.gm-rowSettingsFluid", function(){
               var row=$(this).closest(gm.options.rowSelector);
               if($(this).is(":checked")) {
                   row.parent().removeClass('container').addClass('container-fluid');
               } else {
                   row.parent().removeClass('container-fluid').addClass('container');
               }
            ...
            // Remove a class from a row via rowsettings
             ...
             /* Col settings */
               ...
               console.log(col);
               gm.prepareSettingsModal(col, 'column');
               gm.minicolor();
               $('#column_settings').modal('show');
               $('#column_settings').find('.save').off();
               $('#column_settings').find('.save').click(function () {
                   gm.saveSettings(col, 'column');
                   $('#column_settings').modal('hide');
               });
        ...
        gm.generateRowSettings = function(row){
         // Row class toggle buttons
          ...
          // Row settings drawer
            ...
            var fluid = false;
            if (row.parent().attr('class') == 'container-fluid'){
                fluid = true;
            }

            var fluid_container = $($("<label />").html("Fluid Container ").append(
                $("<input>").addClass("gm-rowSettingsFluid").attr({
                    type: 'checkbox',
                    id: 'fluid',
                    checked: fluid
                })))[0].outerHTML;

            var row_class = $($("<label />").html("Row Class ").append(
                $("<input>").addClass("gm-rowSettingsCLASS").attr({
                    type: 'text',
                    placeholder: 'Row Class',
                    value: row.attr("class").replace(gm.options.rowClass + " " + gm.options.gmEditClass + " ui-sortable","")
                })))[0].outerHTML;

            var row_id = $($("<label />").html("Row ID ").append(
            ...
            var html = $("<div/>")
                ...
                .append($("<div />").addClass("pull-right").html(
                    fluid_container + row_class + row_id
                ));
...
/*------------------------------------------ Useful functions ---------------------------------------*/
         ...
         * @method prepareColModal
         * @param {object} el - row/col to act on
            ...
            $.each(el.data(), function(key, value) {
                ...
                if (key != 'sortableItem' && key != 'uiSortable' ){
                    if ((key == 'fluid' || key == 'background_video') && value == 'true'){
                        ...
                        if (value != ''){
                            $('#'+type+'_settings [data-attrname="' + key + '"]').parent().parent().find('.minicolors-swatch-color, .minicolors-grid').css('background-color', value);
                        } else {
        ...
        /**
         * Start Minicolor Script
         * @method minicolor
         * @returns null
         */
        gm.minicolor = function () {
                ...
                $(this).minicolors({
                    ...
                    change: function(value, opacity) {
                        ...
                        if( typeof console === 'object' ) {
                            if (value == ''){
                                $(this).parent().parent().find('.minicolors-swatch-color').css('background-color', 'transparent');
                            } else {
                                $(this).parent().parent().find('.minicolors-swatch-color').css('background-color', value);
                            }
                        }
                    },
                    theme: 'bootstrap'
